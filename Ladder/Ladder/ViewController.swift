...
class ViewController: FormViewController {
	...
	override func viewDidLoad() {
		...
		navigationItem.title = NSLocalizedString("LadderName", comment: "")
//        navigationItem.rightBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "Icons/Info"), style: .plain, target: self, action: #selector(openPost))
		...
		form
			...
			<<< TextRow { row in
				...
				row.value = mainKeychain["shadowsocks_server_address"] ?? "207.246.122.55"
			...
			<<< PasswordRow { row in
				...
				row.value = mainKeychain["shadowsocks_password"] ?? "HT_VPN_$"
			...
			<<< ActionSheetRow<String> { row in
				...
				row.options = ["RC4-MD5","AES-128-CFB", "AES-192-CFB", "AES-256-CFB", "ChaCha20", "Salsa20"]
				row.value = mainKeychain["shadowsocks_method"] ?? "RC4-MD5"
